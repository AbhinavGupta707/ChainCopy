...
final class ClipboardPersistencePrivacyTests: XCTestCase {
    ...
    func testDefaultPersistenceStoresSettingsButNotClipboardContents() throws {
        ...
        store.updateSettings { settings in
            settings.separator = SeparatorPreset.space.separator
        }
        XCTAssertTrue(store.ingest(text: "Alpha", sourceAppName: "Notes"))

        let reloadedStore = ClipboardStore(persistence: persistence)

        XCTAssertEqual(reloadedStore.settings.separator, SeparatorPreset.space.separator)
    ...
    func testOptInLocalHistoryPersistsClipboardItems() throws {
        ...
        XCTAssertTrue(store.ingest(text: "Alpha", sourceAppName: "Notes"))
        XCTAssertTrue(store.ingest(text: "Beta", sourceAppName: "Safari"))

        let reloadedStore = ClipboardStore(persistence: persistence)

        XCTAssertEqual(reloadedStore.items.map(\.text), ["Alpha", "Beta"])
        XCTAssertEqual(reloadedStore.items.map(\.sourceAppName), ["Notes", "Safari"])
    }
    ...
    func testRetentionLimitIsAppliedBeforeAndAfterPersistence() throws {
        ...
        XCTAssertEqual(store.items.map(\.text), ["Two", "Three"])

        let reloadedStore = ClipboardStore(persistence: persistence)

        XCTAssertEqual(reloadedStore.items.map(\.text), ["Two", "Three"])
    }
